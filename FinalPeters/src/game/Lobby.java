...
public class Lobby extends JPanel {
	...
	private void buildLobby() {
		...
		send_button.addActionListener(e -> {
			...
			Packet pack = new Packet01Message(client_name + "," + msg, this.game.getClient().getClientID());
		...
		ready_button.addActionListener(e -> {
			Packet ready = new Packet02Ready(client_name, game.getClient().getClientID());
		...
		WindowListener exit_listener = new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				Packet disconnect = new Packet11Disconnect(client_name, game.getClient());
